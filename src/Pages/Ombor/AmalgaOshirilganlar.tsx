...
function entityNomi(
  ...
  biriktirilmaganMatni: string
) {
  const entity = boglangan ?? (id ? xarita.get(id) : undefined);
  return entity?.fullName?.trim() || entity?.name?.trim() || entity?.username?.trim() || biriktirilmaganMatni;
}
...
function statusMatni(value: string | undefined, t: (key: string) => string) {
  const status = String(value ?? "DRAFT").toUpperCase();
  return t(`umumiy.status.${status}`);
}
    ...
    () =>
      store.sotuvlar
        ...
        .map((sotuv) => ({
          ...
          ombor: omborNomi(sotuv.warehouseId, sotuv.warehouse, omborMap, t("amalgaOshirilganlar.unknownWarehouse")),
          masul: entityNomi(sotuv.responsibleId, sotuv.responsible, xodimMap, t("umumiy.biriktirilmagan")),
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-slate-950">{t("amalgaOshirilganlar.title")}</h1>
          <p className="mt-1 text-sm leading-6 text-slate-500">
            {t("amalgaOshirilganlar.subtitle")}
          </p>
        </div>
        ...
        <div className="relative shrink-0">
          ...
          >
            <Plus size={18} /> {t("amalgaOshirilganlar.createButton")} <ChevronDown size={15} />
          </button>
          {yaratishOchiq && (
            <div className="absolute right-0 top-14 z-30 w-56 overflow-hidden rounded-2xl border border-orange-100 bg-white p-2 shadow-xl">
              {yaratishVariantlari.map((variant) => (
                ...
                >
                  {t(`amalgaOshirilganlar.createOptions.${variant.key}`)}
                </button>
              ))}
            </div>
          )}
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <label className="relative block w-full max-w-[480px]">
          ...
          <input
            ...
            placeholder={t("amalgaOshirilganlar.searchPlaceholder")}
              ...
              <tr>
                ...
                <th className="w-[76px] px-5 py-4 text-right">
                  <button
                    ...
                    title={t("umumiy.ustunlarniSozlash")}
        ...
        {sozlamaOchiq && (
          <div className="absolute right-4 top-14 z-40 w-56 rounded-2xl border border-orange-100 bg-white p-3 shadow-xl">
            <p className="px-2 pb-2 text-xs font-black uppercase tracking-wide text-slate-400">
              {t("amalgaOshirilganlar.visibleColumns")}
            </p>
            {ustunlar.map((ustun) => (
              <label
                key={ustun.kalit}
