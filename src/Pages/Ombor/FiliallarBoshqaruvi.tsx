...
export default function FiliallarBoshqaruvi() {
  ...
  return <div className="space-y-5">
    ...
    {modal&&<AppModal><form onSubmit={saqlash} className="w-full max-w-lg rounded-[28px] bg-white p-6 shadow-2xl"><h2 className="text-2xl font-black">{editing?"Filialni tahrirlash":"Yangi filial"}</h2>{store.xatolik&&<div className="mt-4 rounded-xl bg-red-50 p-3 text-sm font-bold text-red-600">{store.xatolik}</div>}<div className="mt-5 space-y-3"><input value={name} onChange={e=>setName(e.target.value)} className="h-12 w-full rounded-2xl border px-4" placeholder="Filial nomi *"/><textarea value={address} onChange={e=>setAddress(e.target.value)} className="w-full rounded-2xl border p-4" placeholder="Manzil"/><AppSelect value={status} onChange={e=>setStatus(e.target.value as "ACTIVE"|"INACTIVE")} className="h-12 w-full rounded-2xl border px-4"><option value="ACTIVE">Faol</option><option value="INACTIVE">Faol emas</option></AppSelect><AppSelect value={responsibleId} onChange={e=>setResponsibleId(e.target.value)} className="h-12 w-full rounded-2xl border px-4"><option value="">Mas'ul biriktirilmagan</option>{store.xodimlar.map(x=><option key={x.id} value={x.id}>{x.fullName??x.username??x.id}</option>)}</AppSelect></div><div className="mt-6 flex justify-end gap-3"><button type="button" onClick={()=>setModal(false)} className="h-11 rounded-2xl bg-gray-100 px-5 font-bold">Yopish</button><button disabled={store.amalBajarilmoqda} className="inline-flex h-11 items-center gap-2 rounded-2xl bg-orange-500 px-5 font-black text-white disabled:opacity-50">{store.amalBajarilmoqda&&<LoaderCircle size={16} className="animate-spin"/>}Saqlash</button></div></form></AppModal>}
  </div>
}
