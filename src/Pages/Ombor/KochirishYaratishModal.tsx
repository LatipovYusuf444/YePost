...
export default function KochirishYaratishModal({ onClose }: Props) {
  ...
  function tekshirish() {
    if (!sourceWarehouseId || !destWarehouseId) return "kochirishYaratishModal.errors.warehousesRequired";
    if (sourceWarehouseId === destWarehouseId) return "kochirishYaratishModal.errors.sameWarehouse";
    if (!items.length || items.some((item) => !item.modificationId || !Number.isFinite(item.quantity) || item.quantity <= 0)) {
      return "kochirishYaratishModal.errors.rowsInvalid";
    }
    if (new Set(items.map((item) => item.modificationId)).size !== items.length) {
      return "kochirishYaratishModal.errors.duplicateProduct";
    }
    const ortiqcha = items.some((item) => {
      const qoldiq = qoldiqlar.find((qator) => qator.modificationId === item.modificationId);
      return !qoldiq || item.quantity > qoldiqMiqdori(qoldiq);
    });
    if (ortiqcha) return "kochirishYaratishModal.errors.exceedsStock";
    return "";
  }
    ...
    <AppModal className="items-start justify-start overflow-hidden bg-slate-950/60 p-0 py-4 pl-[92px] pr-4 backdrop-blur-[3px]">
      <div className="relative h-[calc(100dvh-32px)] w-full">
        <aside className="absolute -left-[58px] top-6 z-30 flex flex-col items-center gap-3">
          <button type="button" onClick={onClose} className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#2563EB] text-white shadow-xl ring-1 ring-white/80" aria-label={t("kochirishYaratishModal.closeAria")}>
            <X size={21} />
          </button>
          <button type="button" onClick={() => window.print()} className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white text-[#2563EB] shadow-lg ring-1 ring-orange-100" aria-label={t("kochirishYaratishModal.printAria")}>
            <Printer size={18} />
          </button>
        </aside>
        ...
        <div className="flex h-full flex-col overflow-hidden rounded-l-[44px] rounded-r-[36px] border border-orange-100 bg-gradient-to-br from-[#F8FAFC] via-[#FFFFFF] to-[#E8EEF7] shadow-[0_35px_110px_rgba(15,23,42,.42)]">
          <header className="flex min-h-[92px] shrink-0 items-center gap-4 border-b border-orange-100 bg-[#F8FAFC]/95 px-8 py-4 backdrop-blur-xl">
            <div className="flex items-center gap-4">
              <h2 className="text-3xl font-black tracking-tight text-slate-950 sm:text-[38px]">{t("kochirishYaratishModal.title")}</h2>
              <Copy size={19} className="text-slate-300" />
              <span className="rounded-full bg-orange-50 px-4 py-1.5 text-xs font-black uppercase text-[#2563EB]">{t("kochirishYaratishModal.badge")}</span>
            </div>
          </header>
          ...
          <div className="scrollbar-hidden min-h-0 flex-1 overflow-y-auto p-6 sm:p-8">
            <div className="grid gap-6 xl:grid-cols-2">
              <section className="rounded-[28px] border border-orange-100 bg-white p-5 shadow-sm sm:p-6">
                <h3 className="border-b border-orange-100 pb-4 text-base font-black uppercase text-slate-600">{t("kochirishYaratishModal.sections.about")}</h3>
                <div className="mt-5 grid gap-5 sm:grid-cols-2">
                  <label className="grid gap-2 text-sm font-bold text-slate-400">
                    {t("kochirishYaratishModal.fields.sourceWarehouse")}
                    <AppSelect value={sourceWarehouseId} onChange={(event) => void manbaTanlash(event.target.value)} className="input text-slate-700">
                      <option value="">{t("kochirishYaratishModal.fields.sourceWarehousePlaceholder")}</option>
                      {store.omborlar.map((ombor) => <option key={ombor.id} value={ombor.id}>{ombor.name}</option>)}
                    </AppSelect>
                  </label>
                  <label className="grid gap-2 text-sm font-bold text-slate-400">
                    {t("kochirishYaratishModal.fields.destWarehouse")}
                    <AppSelect value={destWarehouseId} onChange={(event) => setDest(event.target.value)} className="input text-slate-700">
                      <option value="">{t("kochirishYaratishModal.fields.destWarehousePlaceholder")}</option>
                  ...
                  <label className="grid gap-2 text-sm font-bold text-slate-400 sm:col-span-2">
                    {t("kochirishYaratishModal.fields.createdDate")}
                  ...
                  <label className="grid gap-2 text-sm font-bold text-slate-400 sm:col-span-2">
                    {t("kochirishYaratishModal.fields.responsible")}
                    <AppSelect value={responsibleId} onChange={(event) => setResponsible(event.target.value)} className="input text-slate-700">
                      <option value="">{t("kochirishYaratishModal.fields.responsiblePlaceholder")}</option>
              ...
              <div className="space-y-6">
                ...
                <section className="rounded-[28px] border border-orange-100 bg-white p-5 shadow-sm sm:p-6">
                  ...
                  <div className="mt-5 flex gap-3"><span className="mt-2 h-2.5 w-2.5 shrink-0 rounded-full bg-[#2563EB]" /><div><p className="font-black text-slate-700">{t("kochirishYaratishModal.history.draftCreated")}</p><p className="text-sm font-bold text-slate-400">{hozirgiVaqt()}</p></div></div>
                </section>
              </div>
            </div>
            ...
            <section className="mt-6 rounded-[28px] border border-orange-100 bg-white p-5 shadow-sm sm:p-6">
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-orange-100 pb-4">
                <h3 className="font-black uppercase text-slate-600">{t("kochirishYaratishModal.sections.items")}</h3>
                <button type="button" onClick={() => setItems((oldingi) => [...oldingi, yangiQator()])} disabled={!sourceWarehouseId} className="inline-flex h-11 items-center gap-2 rounded-2xl bg-[#2563EB] px-5 text-sm font-black uppercase text-white shadow-lg shadow-orange-100 disabled:opacity-40">
                  <Plus size={17} /> {t("kochirishYaratishModal.items.addRow")}
                </button>
              </div>
              ...
              <div className="mt-5 flex justify-end"><div className="text-right"><p className="text-xs font-black uppercase text-slate-400">{t("kochirishYaratishModal.items.total")}</p><p className="mt-1 text-2xl font-black text-slate-950">{pul(jami)}</p></div></div>
            </section>

            {(xato || store.xatolik) && <p className="mt-5 rounded-2xl bg-red-50 px-5 py-4 text-sm font-bold text-red-600">{xato ? t(xato) : store.xatolik}</p>}
          </div>

          <footer className="flex shrink-0 justify-end gap-3 border-t border-orange-100 bg-[#F8FAFC]/95 px-8 py-4 backdrop-blur-xl">
            <button type="button" onClick={onClose} disabled={store.amalBajarilmoqda} className="h-12 rounded-2xl bg-slate-100 px-7 font-black text-slate-600 disabled:opacity-50">{t("kochirishYaratishModal.footer.cancel")}</button>
            <button type="button" onClick={() => void saqlashVaYopish(false)} disabled={store.amalBajarilmoqda} className="inline-flex h-12 items-center gap-2 rounded-2xl border border-orange-300 bg-white px-7 font-black text-[#2563EB] disabled:opacity-50">{store.amalBajarilmoqda && !jonatilmoqda ? <LoaderCircle size={17} className="animate-spin" /> : <Save size={17} />} {t("kochirishYaratishModal.footer.save")}</button>
            <button type="button" onClick={() => void saqlashVaYopish(true)} disabled={store.amalBajarilmoqda} className="inline-flex h-12 items-center gap-2 rounded-2xl bg-[#2563EB] px-7 font-black text-white shadow-lg shadow-orange-100 disabled:opacity-50">{store.amalBajarilmoqda && jonatilmoqda ? <LoaderCircle size={17} className="animate-spin" /> : <Send size={17} />} {t("kochirishYaratishModal.footer.saveAndSend")}</button>
          </footer>
        </div>
      </div>
    </AppModal>
