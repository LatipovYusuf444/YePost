  ...
  return <section className="overflow-hidden rounded-[34px] border border-orange-100 bg-gradient-to-br from-[#F8FAFC] via-white to-[#EFF6FF] shadow-[0_28px_90px_rgba(15,23,42,.12)] ring-1 ring-white/80">
    ...
    <div className="overflow-x-auto bg-white/70"><table className="w-full min-w-[900px] text-left text-sm">
      ...
      <tbody className="divide-y divide-orange-100/70">{rows.map((sotuv) => <tr key={sotuv.id} onClick={() => onSotuvniOchish(sotuv)} className="group cursor-pointer transition hover:bg-orange-50/65">
        <td className="px-8 py-5"><p className="font-semibold text-slate-900">#{sotuvRaqami(sotuv)}</p><p className="mt-1 text-xs font-semibold text-slate-400">{sotuv.note || "Izoh mavjud emas"}</p></td>
    ...
    <div className="flex justify-between border-t border-orange-100 bg-[#F8FAFC] px-8 py-4 text-xs font-bold text-slate-500"><span>{rows.length} ta yozuv ko'rsatildi</span><span>Qatorni bosing — real sotuv tafsilotlari ochiladi</span></div>
  </section>;
}
