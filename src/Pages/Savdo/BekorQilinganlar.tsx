...
function Stat({ icon: Icon, label, value, dark = false }: { icon: typeof Ban; label: string; value: string; dark?: boolean }) {
  return <article className={`rounded-[24px] p-5 shadow-lg ${dark ? "bg-orange-500 text-white shadow-orange-200" : "bg-white/90 text-slate-900 ring-1 ring-orange-100"}`}><div className="flex items-center justify-between gap-3"><div><p className={`text-xs font-black uppercase tracking-wide ${dark ? "text-white/70" : "text-slate-400"}`}>{label}</p><p className="mt-2 text-xl font-black">{value}</p></div><span className={`flex h-11 w-11 items-center justify-center rounded-2xl ${dark ? "bg-white/15" : "bg-orange-50 text-orange-500"}`}><Icon size={21}/></span></div></article>;
}
