  ...
  return <section className="overflow-hidden rounded-[34px] border border-orange-100 bg-gradient-to-br from-[#fff8ef] via-white to-[#fff1e3] shadow-[0_28px_90px_rgba(120,53,15,.12)] ring-1 ring-white/80">
    <div className="relative overflow-hidden border-b border-orange-100 px-6 py-8 sm:px-9">
      ...
      <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div><p className="flex items-center gap-2 text-xs font-black uppercase tracking-[.22em] text-orange-500"><ShieldX size={17}/> Savdo nazorati</p><h1 className="mt-2 text-3xl font-black tracking-tight text-slate-950">Bekor qilingan sotuvlar</h1></div>
        <button onClick={() => void yangilash()} disabled={yangilanmoqda} className="inline-flex h-11 items-center justify-center gap-2 rounded-2xl bg-slate-950 px-5 text-sm font-black text-white shadow-xl shadow-slate-300/60 transition hover:-translate-y-0.5 hover:bg-orange-600 disabled:opacity-60"><RefreshCw size={17} className={yangilanmoqda ? "animate-spin" : ""}/> Yangilash</button>
      </div>
      <div className="relative mt-7 grid gap-4 md:grid-cols-3">
        <Stat icon={Ban} label="Bekor qilinganlar" value={`${bekorQilinganlar.length} ta`} dark />
        <Stat icon={WalletCards} label="Bekor qilingan summa" value={pulniFormatlash(jamiSumma)} />
        <Stat icon={CalendarX2} label="Bugun bekor qilingan" value={`${bugungi} ta`} />
      </div>
    </div>
    ...
    <div className="overflow-x-auto bg-white/70"><table className="w-full min-w-[1050px] text-left text-sm">
      ...
      <tbody className="divide-y divide-orange-100/70">{rows.map((sotuv) => <tr key={sotuv.id} onClick={() => onSotuvniOchish(sotuv)} className="group cursor-pointer transition hover:bg-orange-50/65">
        ...
        <td className="px-5 py-5 font-black text-slate-900">{pulniFormatlash(sotuvSummasi(sotuv))}</td><td className="px-5 py-5 font-semibold text-slate-600">{sananiFormatlash(bekorSana(sotuv))}</td><td className="px-5 py-5 font-bold text-slate-700">{masulNomi(sotuv)}</td><td className="px-5 py-5"><span className="inline-flex rounded-full bg-red-50 px-3 py-1.5 text-xs font-black text-red-600 ring-1 ring-red-100">Bekor qilingan</span></td><td className="px-8 py-5 text-right"><button onClick={(e) => { e.stopPropagation(); onSotuvniOchish(sotuv); }} className="inline-flex h-10 items-center gap-2 rounded-2xl bg-slate-950 px-4 text-xs font-black text-white transition group-hover:bg-orange-500"><Eye size={15}/> Batafsil</button></td>
      </tr>)}</tbody>
    </table></div>
...
function Stat({ icon: Icon, label, value, dark = false }: { icon: typeof Ban; label: string; value: string; dark?: boolean }) {
  return <article className={`rounded-[24px] p-5 shadow-lg ${dark ? "bg-slate-950 text-white shadow-slate-300/60" : "bg-white/90 text-slate-900 ring-1 ring-orange-100"}`}><div className="flex items-center justify-between gap-3"><div><p className={`text-xs font-black uppercase tracking-wide ${dark ? "text-white/55" : "text-slate-400"}`}>{label}</p><p className="mt-2 text-xl font-black">{value}</p></div><span className={`flex h-11 w-11 items-center justify-center rounded-2xl ${dark ? "bg-white/10 text-orange-400" : "bg-orange-50 text-orange-500"}`}><Icon size={21}/></span></div></article>;
}
