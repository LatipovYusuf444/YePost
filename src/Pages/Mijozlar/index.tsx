...
function PartnerModal({tab,item,onClose}:{tab:Tab;item:Partner|"new";onClose:()=>void}) {
  ...
  const [firstName,setFirstName]=useState(customer?.firstName??"");const [lastName,setLastName]=useState(customer?.lastName??"");const [middleName,setMiddleName]=useState(customer?.middleName??"");const [phone,setPhone]=useState(customer?.phone??company?.phone??supplier?.phone??"");const [address,setAddress]=useState(customer?.address??"");const [telegramId,setTelegramId]=useState(customer?.telegramId??"");const [companyId,setCompanyId]=useState(customer?.companyId??"");const [name,setName]=useState(company?.name??supplier?.name??"");const [inn,setInn]=useState(company?.inn??"");
  async function save(e:FormEvent){e.preventDefault();let ok=false;if(tab==="mijozlar"){if(!firstName.trim()||!lastName.trim()||!phone.trim())return;ok=await store.mijozSaqlash(editing?item.id:null,{firstName:firstName.trim(),lastName:lastName.trim(),middleName:middleName.trim()||undefined,phone:phone.trim(),address:address.trim()||undefined,telegramId:telegramId.trim()||undefined,companyId:companyId||undefined})}else if(tab==="kompaniyalar"){if(!name.trim())return;ok=await store.kompaniyaSaqlash(editing?item.id:null,{name:name.trim(),inn:inn.trim()||undefined,phone:phone.trim()||undefined})}else{if(!name.trim())return;ok=await store.yetkazibBeruvchiSaqlash(editing?item.id:null,{name:name.trim(),phone:phone.trim()||undefined})}if(ok)onClose()}
  const title=`${editing?"Tahrirlash":"Yangi"} ${tab==="mijozlar"?"mijoz":tab==="kompaniyalar"?"mijoz kompaniyasi":"yetkazib beruvchi"}`;
  return <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/55 p-4 backdrop-blur-sm"><form onSubmit={save} className="max-h-[94vh] w-full max-w-2xl overflow-y-auto rounded-[30px] bg-white p-6"><div className="flex justify-between"><div><p className="text-sm font-bold uppercase tracking-wider text-orange-500">Hamkor ma'lumotlari</p><h2 className="text-2xl font-black">{title}</h2></div><button type="button" onClick={onClose} className="flex h-10 w-10 items-center justify-center rounded-xl bg-gray-100"><X size={18}/></button></div>
    {store.xatolik&&<div className="mt-4 rounded-xl bg-red-50 p-3 font-bold text-red-600">{store.xatolik}</div>}
    <div className="mt-5 grid gap-4 sm:grid-cols-2">
      {tab==="mijozlar"?<><input value={firstName} onChange={e=>setFirstName(e.target.value)} className="input" placeholder="Ism *"/><input value={lastName} onChange={e=>setLastName(e.target.value)} className="input" placeholder="Familiya *"/><input value={middleName} onChange={e=>setMiddleName(e.target.value)} className="input" placeholder="Otasining ismi"/><input value={phone} onChange={e=>setPhone(e.target.value)} className="input" placeholder="Telefon *"/><input value={telegramId} onChange={e=>setTelegramId(e.target.value)} className="input" placeholder="Telegram ID"/><select value={companyId} onChange={e=>setCompanyId(e.target.value)} className="input"><option value="">Kompaniya biriktirilmagan</option>{store.kompaniyalar.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select><textarea value={address} onChange={e=>setAddress(e.target.value)} className="min-h-24 rounded-2xl border p-4 sm:col-span-2" placeholder="Manzil"/></>:<><input value={name} onChange={e=>setName(e.target.value)} className="input sm:col-span-2" placeholder="Nomi *"/>{tab==="kompaniyalar"&&<input value={inn} onChange={e=>setInn(e.target.value)} className="input" placeholder="STIR"/>}<input value={phone} onChange={e=>setPhone(e.target.value)} className="input" placeholder="Telefon"/></>}
    </div>
