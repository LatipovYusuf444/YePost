...
import RealizatsiyaKorishModal from "./RealizatsiyaKorishModal";
import RealizatsiyaModal from "./RealizatsiyaModal";
import { boshlangichRealizatsiyalar, mockMahsulotlar, mockOmborlar } from "./mockData";
import type { RealizatsiyaHujjat } from "./types";
import { holatNomi, realizatsiyaJami, omborNomi, pul, sana, tarixgaQoshish } from "./yordamchilar";

const BARCHA_USTUNLAR = [
  { kalit: "nomi", nom: "Nomi" },
  { kalit: "mijoz", nom: "Mijoz" },
  { kalit: "kompaniya", nom: "Kompaniya" },
  { kalit: "savdoId", nom: "Savdo raqami" },
  { kalit: "sana", nom: "Sana" },
...
const SAQLASH_KALITI = "omboruchot-realizatsiya-ustunlar";
...
export default function RealizatsiyaJadvali() {
  const [royxat, setRoyxat] = useState<RealizatsiyaHujjat[]>(boshlangichRealizatsiyalar);
  const [qidiruv, setQidiruv] = useState("");
  const [modalOchiq, setModalOchiq] = useState(false);
  const [tahrirHujjat, setTahrirHujjat] = useState<RealizatsiyaHujjat | null>(null);
  const [korishHujjat, setKorishHujjat] = useState<RealizatsiyaHujjat | null>(null);
  const [ochirilayotganHujjat, setOchirilayotganHujjat] = useState<RealizatsiyaHujjat | null>(null);
  ...
  const keyingiNomi = useMemo(() => `Realizatsiya hujjati #${royxat.length + 1}`, [royxat.length]);
  ...
  const korinadiganRoyxat = useMemo(() => {
    ...
    return royxat.filter(
      (hujjat) =>
        ...
        hujjat.kompaniya.toLowerCase().includes(soz) ||
        (hujjat.savdoId ?? "").toLowerCase().includes(soz) ||
  ...
  function modalniOchish(hujjat?: RealizatsiyaHujjat) {
    setTahrirHujjat(hujjat ?? null);
    setModalOchiq(true);
  }

  function saqlash(hujjat: RealizatsiyaHujjat, tasdiqla: boolean) {
    ...
    const yakuniyHujjat: RealizatsiyaHujjat = {
      ...hujjat,
  ...
  function ochirishSorash(hujjat: RealizatsiyaHujjat) {
  ...
  function bekorQilish(hujjat: RealizatsiyaHujjat) {
    const yangilangan: RealizatsiyaHujjat = {
      ...hujjat,
      holati: "qoralama",
  ...
  function tasdiqlash(hujjat: RealizatsiyaHujjat) {
    const yangilangan: RealizatsiyaHujjat = {
      ...hujjat,
      holati: "tasdiqlangan",
  ...
  function katakQiymati(hujjat: RealizatsiyaHujjat, kalit: UstunKaliti) {
    switch (kalit) {
      case "nomi":
      ...
      case "kompaniya":
        return <span className="text-gray-700">{hujjat.kompaniya || "—"}</span>;
      case "savdoId":
        return <span className="font-bold text-orange-500">{hujjat.savdoId || "—"}</span>;
      case "sana":
      ...
      case "summa":
        return <span className="font-bold text-gray-700">{pul(realizatsiyaJami(hujjat))}</span>;
      default:
        return null;
    }
  }
    ...
    <div className="space-y-5">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-black text-gray-950">Realizatsiya</h1>
          <p className="mt-1 text-sm text-gray-500">
            Savdo orqali sotilgan tovarlar ombordan chiqim qilinadi. Hujjat savdo amalga oshirilganda
      ...
      <div className="relative max-w-sm">
        ...
        <input
          ...
          placeholder="Nomi, mijoz, kompaniya, savdo raqami yoki holati bo'yicha qidirish"
      ...
      <div className="overflow-hidden rounded-[24px] border border-orange-100 bg-white shadow-sm">
        <div className="flex">
          ...
          <div
            className="shrink-0"
            style={{ width: AMALLAR_KENGLIK_PX }}
          >
        ...
        {korinadiganRoyxat.length === 0 && (
          <div className="p-14 text-center">
            ...
            <p className="mt-3 font-bold text-gray-500">Realizatsiya hujjati topilmadi</p>
            <p className="mt-1 text-sm text-gray-400">
              Savdo qilinganda hujjat avtomatik paydo bo'ladi yoki "Yaratish" tugmasi orqali qo'lda qo'shing.
            </p>
          </div>
        )}
      </div>
