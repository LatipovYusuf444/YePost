...
import KochirmaKorishModal from "./KochirmaKorishModal";
import KochirmaModal from "./KochirmaModal";
import { boshlangichKochirmalar, mockMahsulotlar, mockOmborlar } from "./mockData";
import type { KochirmaHujjat } from "./types";
import { holatNomi, kochirmaJami, omborNomi, pul, sana, tarixgaQoshish } from "./yordamchilar";
...
const BARCHA_USTUNLAR = [
  ...
  { kalit: "omborIdFrom", nom: "Ombordan" },
  { kalit: "omborIdTo", nom: "Omborga" },
...
const SAQLASH_KALITI = "omboruchot-kochirma-ustunlar";
...
export default function KochirmaJadvali() {
  const [royxat, setRoyxat] = useState<KochirmaHujjat[]>(boshlangichKochirmalar);
  const [qidiruv, setQidiruv] = useState("");
  const [modalOchiq, setModalOchiq] = useState(false);
  const [tahrirHujjat, setTahrirHujjat] = useState<KochirmaHujjat | null>(null);
  const [korishHujjat, setKorishHujjat] = useState<KochirmaHujjat | null>(null);
  const [ochirilayotganHujjat, setOchirilayotganHujjat] = useState<KochirmaHujjat | null>(null);
  ...
  const keyingiNomi = useMemo(() => `Ko'chirma hujjati #${royxat.length + 1}`, [royxat.length]);
  ...
  const korinadiganRoyxat = useMemo(() => {
    ...
    return royxat.filter(
      (hujjat) =>
        ...
        omborNomi(mockOmborlar, hujjat.omborIdFrom).toLowerCase().includes(soz) ||
        omborNomi(mockOmborlar, hujjat.omborIdTo).toLowerCase().includes(soz) ||
        sana(hujjat.sana).toLowerCase().includes(soz) ||
        holatNomi(hujjat.holati).toLowerCase().includes(soz)
    );
  }, [royxat, qidiruv]);

  function modalniOchish(hujjat?: KochirmaHujjat) {
    setTahrirHujjat(hujjat ?? null);
    setModalOchiq(true);
  }

  function saqlash(hujjat: KochirmaHujjat, tasdiqla: boolean) {
    ...
    const yakuniyHujjat: KochirmaHujjat = {
      ...hujjat,
  ...
  function ochirishSorash(hujjat: KochirmaHujjat) {
  ...
  function bekorQilish(hujjat: KochirmaHujjat) {
    const yangilangan: KochirmaHujjat = {
      ...hujjat,
      holati: "qoralama",
  ...
  function tasdiqlash(hujjat: KochirmaHujjat) {
    const yangilangan: KochirmaHujjat = {
      ...hujjat,
      holati: "tasdiqlangan",
  ...
  function katakQiymati(hujjat: KochirmaHujjat, kalit: UstunKaliti) {
    switch (kalit) {
      case "nomi":
      ...
      case "omborIdFrom":
        return <span className="text-gray-700">{omborNomi(mockOmborlar, hujjat.omborIdFrom)}</span>;
      case "omborIdTo":
        return <span className="text-gray-700">{omborNomi(mockOmborlar, hujjat.omborIdTo)}</span>;
      case "summa":
        return <span className="font-bold text-gray-700">{pul(kochirmaJami(hujjat))}</span>;
      default:
        return null;
    }
  }
    ...
    <div className="space-y-5">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-black text-gray-950">Ko'chirma</h1>
          <p className="mt-1 text-sm text-gray-500">Omborlar orasida tovar ko'chirish hujjatlari.</p>
        </div>
        <button
      ...
      <div className="relative max-w-sm">
        ...
        <input
          ...
          placeholder="Nomi, mas'ul shaxs, ombor, sana yoki holati bo'yicha qidirish"
      ...
      <div className="overflow-hidden rounded-[24px] border border-orange-100 bg-white shadow-sm">
        ...
        {korinadiganRoyxat.length === 0 && (
          <div className="p-14 text-center">
            ...
            <p className="mt-3 font-bold text-gray-500">Ko'chirma hujjati topilmadi</p>
            <p className="mt-1 text-sm text-gray-400">"Yaratish" tugmasi orqali yangi ko'chirma qo'shing.</p>
          </div>
        )}
      </div>
