...
import KirimKorishModal from "./KirimKorishModal";
import KirimModal from "./KirimModal";
import { boshlangichKirimlar, mockMahsulotlar, mockOmborlar } from "./mockData";
import type { KirimHujjat } from "./types";
import { holatNomi, kirimJami, omborNomi, pul, sana, tarixgaQoshish } from "./yordamchilar";

const BARCHA_USTUNLAR = [
  { kalit: "nomi", nom: "Nomi" },
  { kalit: "yetkazibBeruvchi", nom: "Yetkazib beruvchi" },
  { kalit: "sana", nom: "Sana" },
...
const SAQLASH_KALITI = "omboruchot-kirim-ustunlar";
...
export default function KirimJadvali() {
  const [royxat, setRoyxat] = useState<KirimHujjat[]>(boshlangichKirimlar);
  const [qidiruv, setQidiruv] = useState("");
  const [modalOchiq, setModalOchiq] = useState(false);
  const [tahrirHujjat, setTahrirHujjat] = useState<KirimHujjat | null>(null);
  const [korishHujjat, setKorishHujjat] = useState<KirimHujjat | null>(null);
  const [ochirilayotganHujjat, setOchirilayotganHujjat] = useState<KirimHujjat | null>(null);
  ...
  const keyingiNomi = useMemo(() => `Kirim hujjati #${royxat.length + 1}`, [royxat.length]);
  ...
  const korinadiganRoyxat = useMemo(() => {
    ...
    return royxat.filter(
      (hujjat) =>
        ...
        hujjat.yetkazibBeruvchi.toLowerCase().includes(soz) ||
  ...
  function modalniOchish(hujjat?: KirimHujjat) {
    setTahrirHujjat(hujjat ?? null);
    setModalOchiq(true);
  }

  function saqlash(hujjat: KirimHujjat, tasdiqla: boolean) {
    ...
    const yakuniyHujjat: KirimHujjat = {
      ...hujjat,
  ...
  function ochirishSorash(hujjat: KirimHujjat) {
  ...
  function bekorQilish(hujjat: KirimHujjat) {
    const yangilangan: KirimHujjat = {
      ...hujjat,
      holati: "qoralama",
  ...
  function tasdiqlash(hujjat: KirimHujjat) {
    const yangilangan: KirimHujjat = {
      ...hujjat,
      holati: "tasdiqlangan",
  ...
  function katakQiymati(hujjat: KirimHujjat, kalit: UstunKaliti) {
    switch (kalit) {
      case "nomi":
        return <span className="font-black text-gray-950">{hujjat.nomi}</span>;
      case "yetkazibBeruvchi":
        return <span className="text-gray-700">{hujjat.yetkazibBeruvchi || "—"}</span>;
      case "sana":
      ...
      case "summa":
        return <span className="font-bold text-gray-700">{pul(kirimJami(hujjat))}</span>;
      default:
        return null;
    }
  }
      ...
      <div className="overflow-hidden rounded-[24px] border border-orange-100 bg-white shadow-sm">
        ...
        {korinadiganRoyxat.length === 0 && (
          <div className="p-14 text-center">
            ...
            <p className="mt-1 text-sm text-gray-400">"Yaratish" tugmasi orqali yangi kirim qo'shing.</p>
          </div>
        )}
      </div>
