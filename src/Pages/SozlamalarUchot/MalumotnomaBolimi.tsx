...
function OlchovBirligiRoyxati() {
  ...
  return <div className="min-w-0">
    ...
    <div className="mb-4 flex flex-col gap-2 sm:flex-row">
      <input value={kod} onChange={(e) => setKod(e.target.value)} placeholder="Kod (796)" className={`${maydonKlass} sm:max-w-[150px]`} />
    ...
    {yuklanmoqda ? <div className="flex h-32 items-center justify-center gap-2 text-sm font-bold text-slate-400"><LoaderCircle className="animate-spin" size={18}/>Yuklanmoqda...</div> : <div className="divide-y divide-orange-100 overflow-hidden rounded-2xl border border-orange-100">
      {olchovlar.map((birlik) => <div key={birlik.id} className="flex items-center gap-3 px-4 py-3 hover:bg-orange-50/40"><span className="flex h-9 min-w-12 items-center justify-center rounded-xl bg-orange-50 px-2 text-xs font-black text-[#FF6A00]">{birlik.code || <Ruler size={16}/>}</span><div className="min-w-0 flex-1"><p className="font-black text-slate-800">{birlik.name}</p><p className="text-xs font-bold text-slate-400">{birlik.shortName || "Qisqa nom berilmagan"}</p></div><button type="button" onClick={() => boshla(birlik)} className="rounded-lg px-3 py-1.5 text-xs font-bold text-[#FF6A00] hover:bg-orange-100">Tahrirlash</button><button type="button" onClick={() => void ochirish(birlik)} className="flex h-8 w-8 items-center justify-center rounded-lg bg-red-50 text-red-500 hover:bg-red-100"><Trash2 size={15}/></button></div>)}
